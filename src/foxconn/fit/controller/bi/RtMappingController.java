...
public class RtMappingController extends BaseController{
	...
	public String index(HttpServletRequest request, Model model) {
		...
		List<String> supplierList = masterDataService.listBySql("select t.lov_code||','||t.tab_name||'|'||t.lov_desc from CUX_MD_LOV_VALUES t where t.lov_type='RT_ACCOUNT' and t.enabled_flag='Y' and t.language='"+language+"' ORDER BY to_number(COL_SEQ)");
	...
	public String queryMasterData(HttpServletRequest request,HttpServletResponse response,AjaxResult result,String masterData){
		try {
			...
			List<List<String>> queryList = masterDataService.listBySql(sql);
	...
	public String upload(HttpServletRequest request,AjaxResult result,@Log(name = "營收映射表") String masterData) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				if (rowNum<4) {
					result.put("flag", "fail");
					result.put("msg", getLanguage(locale,"未發現需要處理的數據","No valid data"));
					return result.getJson();
				}
				...
				Assert.isTrue(tableName.equals(tableNAME), getLanguage(locale,"營收映射表選擇錯誤","The selected master data is error"));
				...
				for (int i = 4; i < rowNum; i++) {
					...
					for (Integer index : indexList) {
						...
						if ("WS".equals(readMap.get(index)) && StringUtils.isNotEmpty(value)) {
							...
							Assert.hasText(value, getLanguage(locale,"後臺配置已更新，請重新下載","The background configuration has changed,please download again"));
						}
 ...
 */
	...
	@ResponseBody
	public String delete(HttpServletRequest request,HttpServletResponse response,AjaxResult result,@Log(name = "營收映射表数据") String masterData,@Log(name = "更新条件") String updateData){
		try {
			Locale locale = (Locale) WebUtils.getSessionAttribute(request,SessionLocaleResolver.LOCALE_SESSION_ATTRIBUTE_NAME);
			Assert.hasText(masterData, getLanguage(locale,"營收映射表不能为空","Master data can not be null"));
			String tableName=masterData.split(",")[1];
	...
	@ResponseBody
	public String insert(HttpServletRequest request,HttpServletResponse response,AjaxResult result,@Log(name = "營收映射表数据單個新增") String formVal,String type){
		try {
