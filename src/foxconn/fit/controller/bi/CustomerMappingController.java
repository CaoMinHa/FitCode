...
import foxconn.fit.service.bi.CustomerMappingService;
import foxconn.fit.util.ExcelUtil;
import foxconn.fit.util.ExceptionUtil;

@Controller
@RequestMapping("/bi/customerMapping")
public class CustomerMappingController extends BaseController {

	private static String EXCEL_NAME="客戶對應表";
	...
	public String index(Model model) {
		return "/bi/customerMapping/index";
	}
	...
	public String list(Model model, PageRequest pageRequest) {
		try {
			...
			Page<CustomerMapping> page = customerMappingService.findPageByHQL(pageRequest, filters);
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询客戶對應表列表失败:", e);
		}
		return "/bi/customerMapping/list";
	}
	...
	public String upload(HttpServletRequest request,HttpServletResponse response, AjaxResult result) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				List<CustomerMapping> list=new ArrayList<CustomerMapping>();
				...
				for (int i = 1; i < rowNum; i++) {
					...
					int n=0;
					CustomerMapping detail=new CustomerMapping();
					detail.setCustomer(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setCustomerGroup(ExcelUtil.getCellStringValue(row.getCell(n++),i));

					list.add(detail);
				}
				
				if (!list.isEmpty()) {
					customerMappingService.saveBatch(list);
				}else{
					result.put("flag", "fail");
	...
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
			pageRequest.setOrderBy("customer");
			pageRequest.setOrderDir(PageRequest.Sort.ASC);
			List<CustomerMapping> list = customerMappingService.listByHQL(filters, pageRequest);
			...
			if (CollectionUtils.isNotEmpty(list)) {
				...
				for (int i = 0; i < list.size(); i++) {
					CustomerMapping detail = list.get(i);
