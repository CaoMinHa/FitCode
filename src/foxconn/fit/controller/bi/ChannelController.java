...
import foxconn.fit.service.bi.ChannelService;
...
public class ChannelController extends BaseController {
	...
	private static String EXCEL_NAME="渠道映射";
	...
	public String index(Model model) {
		List<String> list = channelService.listBySql("select distinct corporation_code as code from FIT_Channel order by corporation_code");
		model.addAttribute("codelist", list);
		return "/bi/channel/index";
	}
	...
	public String list(Model model, PageRequest pageRequest,String corporationCode) {
		try {
			...
			Page<Channel> page = channelService.findPageByHQL(pageRequest, filters);
			
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询渠道映射列表失败:", e);
		}
		return "/bi/channel/list";
	}
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				for (int i = 1; i < rowNum; i++) {
					...
					detail.setCountry(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setGroups(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setClassification1(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setClassification2(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setReceiptCondition(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setPriceCondition(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setTradeCustomer(ExcelUtil.getCellStringValue(row.getCell(n++),i));
	...
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			...
			List<Channel> list = channelService.listByHQL(filters, pageRequest);
