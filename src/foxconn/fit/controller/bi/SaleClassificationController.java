...
import foxconn.fit.service.bi.SaleClassificationService;
import foxconn.fit.util.ExcelUtil;
import foxconn.fit.util.ExceptionUtil;

@Controller
@RequestMapping("/bi/saleClassification")
public class SaleClassificationController extends BaseController {

	private static String EXCEL_NAME="銷售大類映射";
	private static int COLUMN_NUM=3;
	
	@Autowired
	private SaleClassificationService saleClassificationService;

	@RequestMapping(value = "index")
	public String index(Model model) {
		return "/bi/saleClassification/index";
	}
	...
	public String list(Model model, PageRequest pageRequest) {
		try {
			...
			Page<SaleClassification> page = saleClassificationService.findPageByHQL(pageRequest, filters);
			
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询銷售大類映射列表失败:", e);
		}
		return "/bi/saleClassification/list";
	}
	...
	public String upload(HttpServletRequest request,HttpServletResponse response, AjaxResult result) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				List<SaleClassification> list=new ArrayList<SaleClassification>();
				...
				for (int i = 1; i < rowNum; i++) {
					...
					int n=0;
					SaleClassification detail=new SaleClassification();
					...
					detail.setClassification(ExcelUtil.getCellStringValue(row.getCell(n++),i));

					list.add(detail);
				}
				
				if (!list.isEmpty()) {
					saleClassificationService.saveBatch(list);
				}else{
					result.put("flag", "fail");
	...
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
			pageRequest.setOrderBy("customerCode");
			pageRequest.setOrderDir(PageRequest.Sort.ASC);
			List<SaleClassification> list = saleClassificationService.listByHQL(filters, pageRequest);
			...
			if (CollectionUtils.isNotEmpty(list)) {
				...
				for (int i = 0; i < list.size(); i++) {
					SaleClassification detail = list.get(i);
