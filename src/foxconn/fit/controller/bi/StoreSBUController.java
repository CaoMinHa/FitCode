...
import foxconn.fit.service.bi.StoreSBUService;
import foxconn.fit.util.ExcelUtil;
import foxconn.fit.util.ExceptionUtil;

@Controller
@RequestMapping("/bi/storeSBU")
public class StoreSBUController extends BaseController {

	private static String EXCEL_NAME="倉碼&SBU映射";
	...
	public String index(Model model) {
		List<String> list = storeSBUService.listBySql("select distinct SBU from FIT_Store_SBU order by SBU");
		model.addAttribute("codelist", list);
		return "/bi/storeSBU/index";
	}
	...
	public String list(Model model, PageRequest pageRequest,String SBU) {
		try {
			...
			Page<StoreSBU> page = storeSBUService.findPageByHQL(pageRequest, filters);
			
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询倉碼&SBU映射列表失败:", e);
		}
		return "/bi/storeSBU/list";
	}

	@RequestMapping(value = "upload")
	@ResponseBody
	@Log(name = "倉碼&SBU映射-->上传")
	public String upload(HttpServletRequest request,HttpServletResponse response, AjaxResult result) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				List<StoreSBU> list=new ArrayList<StoreSBU>();
				...
				for (int i = 1; i < rowNum; i++) {
					...
					int n=0;
					StoreSBU detail=new StoreSBU();
					detail.setCode(ExcelUtil.getCellStringValue(row.getCell(n++),i));
				...
				if (!list.isEmpty()) {
					storeSBUService.saveBatch(list);
				}else{
					result.put("flag", "fail");
	...
	@ResponseBody
	@Log(name = "倉碼&SBU映射-->下载")
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
			pageRequest.setOrderBy("code");
			pageRequest.setOrderDir(PageRequest.Sort.ASC);
			List<StoreSBU> list = storeSBUService.listByHQL(filters, pageRequest);
