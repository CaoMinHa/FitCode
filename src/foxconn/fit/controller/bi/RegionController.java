...
@Controller
@RequestMapping("/bi/region")
public class RegionController extends BaseController {

	private static String EXCEL_NAME="区域映射";
	...
	public String list(Model model, PageRequest pageRequest) {
		try {
			...
			Page<Region> page = regionService.findPageByHQL(pageRequest, filters);
			
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询区域映射列表失败:", e);
		}
		return "/bi/region/list";
	}
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				for (int i = 1; i < rowNum; i++) {
					...
					detail.setRegion(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setJudgeRegion(ExcelUtil.getCellStringValue(row.getCell(n++),i));
	...
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			...
			List<Region> list = regionService.listByHQL(filters, pageRequest);
