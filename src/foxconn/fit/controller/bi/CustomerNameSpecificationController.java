...
import foxconn.fit.service.bi.CustomerNameSpecificationService;
import foxconn.fit.util.ExcelUtil;
import foxconn.fit.util.ExceptionUtil;

@Controller
@RequestMapping("/bi/customerNameSpecification")
public class CustomerNameSpecificationController extends BaseController {

	private static String EXCEL_NAME="客戶名稱規範映射";
	private static int COLUMN_NUM=6;
	
	@Autowired
	private CustomerNameSpecificationService customerNameSpecificationService;

	@RequestMapping(value = "index")
	public String index(Model model) {
		List<String> list = customerNameSpecificationService.listBySql("select distinct corporation_code as code from FIT_Customer_Name_Specify order by corporation_code");
		model.addAttribute("codelist", list);
		return "/bi/customerNameSpecification/index";
	}
	...
	public String list(Model model, PageRequest pageRequest,String corporationCode) {
		try {
			...
			Page<CustomerNameSpecification> page = customerNameSpecificationService.findPageByHQL(pageRequest, filters);
			
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询客戶名稱規範映射列表失败:", e);
		}
		return "/bi/customerNameSpecification/list";
	}

	@RequestMapping(value = "upload")
	@ResponseBody
	@Log(name = "客戶名稱規範映射-->上传")
	public String upload(HttpServletRequest request,HttpServletResponse response, AjaxResult result) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				List<CustomerNameSpecification> list=new ArrayList<CustomerNameSpecification>();
				...
				for (int i = 1; i < rowNum; i++) {
					...
					int n=0;
					CustomerNameSpecification detail=new CustomerNameSpecification();
					...
					detail.setCustomerShortName(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setSpecificationShortName(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setSpecificationFullName(ExcelUtil.getCellStringValue(row.getCell(n++),i));
					detail.setSpecificationGroupName(ExcelUtil.getCellStringValue(row.getCell(n++),i));

					list.add(detail);
				}
				
				if (!list.isEmpty()) {
					customerNameSpecificationService.saveBatch(list);
				}else{
					result.put("flag", "fail");
	...
	@ResponseBody
	@Log(name = "客戶名稱規範映射-->下载")
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result){
		try {
			List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
			pageRequest.setOrderBy("corporationCode");
			pageRequest.setOrderDir(PageRequest.Sort.ASC);
			List<CustomerNameSpecification> list = customerNameSpecificationService.listByHQL(filters, pageRequest);
			...
			if (CollectionUtils.isNotEmpty(list)) {
				...
				for (int i = 0; i < list.size(); i++) {
					CustomerNameSpecification detail = list.get(i);
