...
$(function() {
	...
	$("a.update").click(function(){
		...
		$.ajax({
			type:"POST",
			url:"${ctx}/admin/masterData/update",
			async:true,
			dataType:"json",
...
function clickPage(page){
	...
	$("#Content").load("${ctx}/admin/masterData/list",{pageNo:$("#PageNo").val(),pageSize:$("#PageSize").val(),
	...
	<table class="table table-condensed table-hover">
		<thead>
			<tr>
				...
				<th style="border-right:1px solid #eee;text-align: center;"><spring:message code='operation'/></th>
			</tr>
		</thead>
		<tbody>
					...
					<c:forEach items="${row }" var="data" varStatus="status">
						<c:choose>
							<c:when test="${status.index eq 0}">
								<td style="border-right:1px solid #eee;display:none;"><input name="ID" type="text" style="display:none;" value="${data}"/></td>
							</c:when>
							<c:otherwise>
								<c:choose>
									<c:when test="${fn:split(data,'|')[1] eq 'W'}">
										<td style="border-right:1px solid #eee;"><input name="${fn:split(data,'|')[0]}" type="text" style="height:25px !important;width:100px;" value="${fn:split(data,'|')[2] }"/></td>
									</c:when>
									...
									<c:otherwise>
										<td style="border-right:1px solid #eee;">${fn:split(data,'|')[2] }</td>
									</c:otherwise>
								</c:choose>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<td style="border-right:1px solid #eee;"><a href="javascript:void(0);" class="update"><spring:message code='update'/></a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
